...
export const AlertShow: React.FC = () => {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      case 'STATUS_CHANGED':
        return <ClockCircleOutlined style={{ color: ACM_COLORS.warning }} />;
      case 'ASSIGNED':
        return <UserOutlined style={{ color: ACM_COLORS.info }} />;
      case 'RESOLVED':
        return <CheckCircleOutlined style={{ color: ACM_COLORS.success }} />;
      case 'NOTE_ADDED':
        return <EditOutlined style={{ color: ACM_COLORS.primary }} />;
      default:
            ...
            <Space direction="vertical" style={{ width: '100%' }}>
              {alert.status !== 'RESOLVED' && (
                <Button
                  ...
                  block
                  style={{ backgroundColor: ACM_COLORS.success }}
                  onClick={() => navigate(`/alerts/edit/${alert.id}`)}
                >
                  Mark as Resolved
                </Button>
